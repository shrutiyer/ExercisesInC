...
![Image of Processes](parent-child.jpg)
...
When the virtual memory addresses of the global, heap and stack segments appear to be the same. But that is because the child process is an exact replica of the parent process. The physical addresses of the parent and child process will be different for the same variables.
...
Running `execle` in the `for` loop replaces the program execution with a different one whose job is done after executing the commands given to it. That is why, the `for` loop never reaches the rest of the RSS feeds.
